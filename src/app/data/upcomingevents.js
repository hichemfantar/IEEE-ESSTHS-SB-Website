//this years events
export const upevents = [
	{
		"Event Title": "Aess Talk",
		"Event Date": "15 Apr 2025 ",
		"State": "Finished",
		"Event_link":"https://www.facebook.com/profile.php?id=61557575908522",
		"Event_image": "https://res.cloudinary.com/dytybjqwb/image/upload/v1742774733/482126767_122192740646252530_8840832326748474967_n_gcxbqt.jpg",
		"Event_pdp": "https://res.cloudinary.com/dytybjqwb/image/upload/v1743120305/aess_talk_q9xprg.png",

		"desrip":"AESS Ramadhan Talks 2.0 (2025): This year, AESS Ramadhan Talks 2.0 returns as an online event featuring a series of expert-led workshops and sessions. Focused on 'Technologies and Communications in Space,' the event will cover key topics such as 5G and space, cybersecurity, IoT, and their implementation in space. It provides a platform for participants to gain valuable insights from industry experts and explore the latest advancements shaping the future of space technology.",
	},
	{
		"Event Title": "Solid works bootcamp",
		"Event Date": "25 Apr 2023 8:30 PM",
		"State": "in Progress",
		"Event_link":"https://www.facebook.com/events/959027069169569/",
		"Event_image": "assets/events/solidworks.jpg",
		"Event_pdp": "https://res.cloudinary.com/dytybjqwb/image/upload/v1743409655/solidwork_dnovue.png",
		"desrip":"SolidWorks Bootcamp (IIP Joint Chapters x AESS SBC x RAS SBC): A collaborative initiative between IIP Joint Chapters, AESS SBC, and RAS SBC, the SolidWorks Bootcamp was established to provide hands-on training in 3D modeling, design, and simulation. Designed for both beginners and advanced learners, this bootcamp has played a key role in equipping participants with the technical skills needed to master SolidWorks through interactive sessions and practical exercises, fostering a community of learners passionate about innovation and design."
	},
	{
		"Event Title": "Innosphere Virtual Summit",
		"Event Date": "25 Apr 2023 8:30 PM",
		"State": "in Progress",
		"Event_link":"https://www.facebook.com/events/959027069169569/",
		"Event_image":
			"https://res.cloudinary.com/dytybjqwb/image/upload/v1742597411/innosphere_yerbap.jpg",
			"Event_pdp": "https://res.cloudinary.com/dytybjqwb/image/upload/v1742597411/innosphere_yerbap.jpg",
			"desrip":"𝐈𝐄𝐄𝐄 𝐈𝐈𝐏 𝐄𝐒𝐒𝐓𝐇𝐒, 𝐈𝐄𝐄𝐄 𝐈𝐀𝐒 𝐈𝐒𝐈 & 𝐈𝐄𝐄𝐄 𝐈𝐀𝐒 𝐈𝐒𝐒𝐀𝐓𝐌 are teaming up for our 𝐈𝐧𝐧𝐨𝐒𝐩𝐡𝐞𝐫𝐞 𝐕𝐢𝐫𝐭𝐮𝐚𝐥 𝐒𝐮𝐦𝐦𝐢𝐭! 💚From 𝟑𝐃 𝐚𝐧𝐢𝐦𝐚𝐭𝐢𝐨𝐧 to 𝐦𝐚𝐜𝐡𝐢𝐧𝐞 𝐥𝐞𝐚𝐫𝐧𝐢𝐧𝐠, 𝐔𝐈/𝐔𝐗, and 𝐏𝐂𝐁 𝐝𝐞𝐬𝐢𝐠𝐧, get ready for a knowledge-packed event starting from 𝐌𝐚𝐫𝐜𝐡 𝟏𝟖𝐭𝐡! 🛠️"
	},
	{
		"Event Title": "IEEE Tejmaana 2.0 ",
		"Event Date": "22 Mar 2025 8:30 PM",
		"State": "Finished",
		"Event_link":"https://www.facebook.com/events/647816040999518/",
		"Event_image":
			"https://res.cloudinary.com/dytybjqwb/image/upload/v1742599018/tejmaana_pk7shk.jpg",
			"Event_pdp": "https://res.cloudinary.com/dytybjqwb/image/upload/v1742599018/tejmaana_pk7shk.jpg",
			"desrip":"IEEE TEJMAANA 2.0 was a distinguished Ramadan event dedicated to promoting the value of IEEE membership, fostering professional networking, and delivering insightful workshops led by IEEE Tunisia Section members. The event commenced with check-in, followed by an opening ceremony, team-building activities, and a communal break-fast, providing an opportunity for meaningful interactions in a collaborative setting. The program featured two parallel workshops focusing on IEEE opportunities and national and international congresses, followed by a panel discussion on 'IEEE & Startup Success,' where experts shared valuable insights on leveraging IEEE for professional growth. A key highlight of the event was the Innovation Challenge, where participants developed innovative solutions to enhance cross-cultural collaboration. After an intensive ideation process, teams presented their proposals to a jury of IEEE Tunisia Section members and experts, who assessed them based on originality, feasibility, and impact. The event concluded with the announcement of the winning teams and a closing ceremony, leaving participants inspired and motivated to further engage in IEEE initiatives."
	},{
		"Event Title": "FrontEnd Development 101",
		"Event Date": "11 March 2025",
		"State": "Finished",
		"Event_link":"https://www.facebook.com/photo/?fbid=938115931818367&set=pb.100068599066993.-2207520000",
		"Event_image": "https://res.cloudinary.com/dytybjqwb/image/upload/v1743414485/Instagram_post_-_52_z7idsu.png",
		"Event_pdp": "https://res.cloudinary.com/dytybjqwb/image/upload/v1743414485/Instagram_post_-_52_z7idsu.png",
		"desrip":"Are you curious about front-end development and how to build stunning, interactive web interfaces? Join us for FrontEnd Development 101, an engaging info session with Hichem Fantar! In this session, you'll get an introduction to the fundamental technologies of front-end development, including HTML, CSS, and JavaScript, and learn about modern frameworks like React. Whether you're a beginner or looking to refine your skills, this is the perfect opportunity to gain valuable insights, ask questions, and get started on your journey as a front-end developer."
	},
];
